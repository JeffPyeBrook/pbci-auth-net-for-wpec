...
class wpec_auth_net_setup {
	...
	function wpsc_init() {
		include_once( 'classes/wpec_auth_net.class.php' );
		...
		if ( is_admin() ) {
			...
			add_filter( 'wpscusermanagement_commit_changes_payment_gateway', array(
				$this,
				'oms_payment_form'
			), 10, 2 );
		...
		if ( $purchlog_data['gateway'] == 'wpec_auth_net' ) {
			...
			if ( ! $auth_net->beenCaptured( $purchlog_id ) ) {
				if ( $auth_net->capturePreAuth( $order ) ) {
					...
					die( __( 'Failed to capture payment', PBCI_AUTHNET ) );
				}
			}
		}
	}
