...
class wpec_auth_net extends wpsc_merchant {
	...
	function __construct( $purchase_id = null, $is_receiving = false ) {
		...
		$this->accountTypes = array(
			'businessChecking' => __( 'Business Checking', PBCI_AUTHNET ),
			'savings'          => __( 'Savings Account', PBCI_AUTHNET ),
			'checking'         => __( 'Checking', PBCI_AUTHNET )
		);
		...
		if ( ! defined( 'AUTHORIZENET_API_LOGIN_ID' ) ) {
			define( 'AUTHORIZENET_API_LOGIN_ID', $this->conf['login'] );
		}

		if ( ! defined( 'AUTHORIZENET_TRANSACTION_KEY' ) ) {
			define( 'AUTHORIZENET_TRANSACTION_KEY', $this->conf['key'] );
		}
	...
	function manage_payment_status() {
		...
		if ( ( ! isset( $show_auth_net_links ) || $show_auth_net_links == false ) && $purchlogitem->extrainfo->gateway == 'wpec_auth_net' ) {
			...
			if ( ! $this->beenCaptured( $purchlogitem->purchlogid ) ) {
				...
				echo "<a href='" . esc_url( add_query_arg( $capture_payment_params ) ) . "'>" . __( 'Accept payment', PBCI_AUTHNET ) . '</a> | ';
			}
			if ( ! isset( $purchlogitem->extrainfo->transactid ) ) {
				//Give Link to capture payment
				echo __( 'Missing Transaction ID in Order', PBCI_AUTHNET ) . " |";
			}
	...
	function displayPurchaseDetails( $purchaselog_id ) {
		...
		$purchaseDetailsText = __( 'Purchase Details', PBCI_AUTHNET );
		$approvedText        = __( 'Approved:', PBCI_AUTHNET );
		$declinedText        = __( 'Declined:', PBCI_AUTHNET );
		$authoCodeText       = __( 'Auth Code:', PBCI_AUTHNET );
		$transactionIdText   = __( 'Transaction ID:', PBCI_AUTHNET );
		$typeText            = __( 'Type:', PBCI_AUTHNET );
		$amountText          = __( 'Ammount:', PBCI_AUTHNET );
		$responseText        = __( 'Response:', PBCI_AUTHNET );
		$md5HashText         = __( 'MD5 Hash:', PBCI_AUTHNET );
...
EOF;
		...
		$title = __( 'Purchase Log Details From Authorize.Net', PBCI_AUTHNET );
...
EOF;
		...
		if ( isset( $this->profile->xml->profile->paymentProfiles ) ) {
			...
			foreach ( $this->profile->xml->profile->paymentProfiles as $account ) {
				if ( isset( $account->payment->creditCard ) ) {
					...
					$output .= "<span class='ccInfo'>" . __( "Card Ending in ", PBCI_AUTHNET ) . $account->payment->creditCard->cardNumber . "<span>\n";
					$output .= "</div>\n";
				}
			}
	...
	function validatePayment() {
		if ( $this->payType == 'creditCardForms' ) {
			...
			if ( ! isset( $creditCard['card_number'] ) ) {
				$this->set_error_message( __( 'Valid Credit Card Not Given.', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
			$this->auth->card_num = $creditCard['card_number'];
			if ( ! isset( $creditCard['expiry'] ) ) {
				$this->set_error_message( __( 'Valid Credit Card Expiration Not Given.', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
			$this->auth->exp_date = $creditCard['expiry']['month'] . $creditCard['expiry']['year'];
			if ( ! isset( $creditCard['card_code'] ) ) {
				$this->set_error_message( __( 'Please Enter The CVV Off The Back of The Card.', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
			...
			if ( ! isset( $bankAccount['bank_name'] ) ) {
				$this->set_error_message( __( 'Please Enter The Bank Name', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
			$check['bank_name'] = $bankAccount['bank_name'];
			if ( ! isset( $bankAccount['account_type'] ) ) {
				$this->set_error_message( __( 'Please Specify The Account Type.', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
			$check['bank_acct_type'] = $bankAccount['account_type'];
			if ( ! isset( $bankAccount['name_on_account'] ) ) {
				$this->set_error_message( __( 'Please Give The Name On The Account.', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
			$check['bank_acct_name'] = $bankAccount['name_on_account'];
			if ( ! isset( $bankAccount['account_number'] ) ) {
				$this->set_error_message( __( 'No Account Number Given.', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
			$check['bank_acct_num'] = $bankAccount['account_number'];
			if ( ! isset( $bankAccount['routing_number'] ) ) {
				$this->set_error_message( __( 'No Bank Routing Number Given.', PBCI_AUTHNET ) );
				$this->return_to_checkout();
			}
	...
	function processCIMTransaction() {
		...
		if ( isGood( $this->purchase_id ) ) {
			$cimTransaction->order->invoiceNumber = $this->purchase_id;
		}
	...
	function processAIMTransaction() {
		...
		$this->auth->customer_ip = wpsc_anet_getip();
		$this->validatePayment();
		$this->setBillingAddress();
		$this->setShippingAddress();

		if ( isGood( $this->purchase_id ) ) {
			$this->auth->invoice_num = $this->purchase_id;
		}

		if ( isGood( $this->cart_data['cart_tax'] ) ) {
			$this->auth->tax = $this->cart_data['cart_tax'];
		}
		if ( isGood( $this->cart_data['base_shipping'] ) ) {
			$this->auth->freight = $this->cart_data['base_shipping'];
		}
	...
	function CheckOrCC() {
		...
		foreach ( $wpsc_cart->cart_items as $item ) {
			if ( $is_recurring = (bool) get_post_meta( $item->product_id, '_wpsc_is_recurring', true ) ) {
				$this->conf['cimon'] = false;
			}
		}

//		$paymentTypesText = array(
//			'select-payment-type-text' 	=> __( 'Select Payment Type', PBCI_AUTHNET ),
//			'credit-card-text' 					=> __( 'Credit Card', PBCI_AUTHNET ),
//			'e-check-text' 							=> __( 'E-Check', PBCI_AUTHNET )
//		);
		...
		<input type='hidden' name='paymentTypes' id='paymentTypes' value='creditCardForms'>
EOF;
		$output = '<div id=\'checkorcc_select\'>';
//		$output .= $this->showCheckForm();
	...
	function showCheckForm() {
		...
		$output .= "<fieldset><legend>" . __( 'E-Check', PBCI_AUTHNET ) . "</legend>";
		if ( isset( $this->conf['cimon'] ) && $this->conf['cimon'] == 'checked' ) {
			$bankProfiles = $this->getBankAccountProfiles();
			if ( $bankProfiles ) {
				$output .= "<div><span class='head2'>" . __( 'Use Bank Account on file.', PBCI_AUTHNET ) . "</span>\n";
				$output .= $bankProfiles;
				$output .= "</div>\n<span class='head2'> " . __( 'Or Enter a New Bank Account', PBCI_AUTHNET ) . "</span>\n";
			}
			$output .= "<span id='saveBankAccount'><input type='checkbox' name='auth_net[SaveBankAccount]' value='Keep On File'>" . __( 'Save Payment Info (You Can Save Up To 10 accounts).', PBCI_AUTHNET ) . "</span>\n";
		}
	...
	function showCCForm() {
		...
		//$output .= "<fieldset><legend>".__('Credit Card',PBCI_AUTHNET)."</legend>";
		if ( isset( $this->conf['cimon'] ) && $this->conf['cimon'] == 'checked' ) {
			$creditcards = $this->getCreditCardProfiles();
			if ( $creditcards ) {
				$output .= "<div><span class='head2'>" . __( 'Use Credit Card on file.', PBCI_AUTHNET ) . "</span>\n";
				$output .= $creditcards;
				$output .= "</div>\n<span class='head2'> " . __( 'Or Enter a New Card', PBCI_AUTHNET ) . "</span>\n</br>";
			}
			$output .= "<span id='saveCreditCard'><input type='checkbox' name='auth_net[SaveCreditCard]' value='Keep On File'>" . __( 'Save Payment Info (You Can Save Up To 10 accounts).', PBCI_AUTHNET ) . "</span>\n";
		}
	...
	function showNewBankAccountForm() {
		if ( isset( $_REQUEST['auth_net']['bankAccount'] ) ) {
			$auth_net = $_REQUEST['auth_net']['bankAccount'];
		} else {
			$auth_net = array(
				'account_type'    => '',
				'bank_name'       => '',
		...
		$bankAccountText = array(
			'bank-name-text'         => __( 'Bank Name', PBCI_AUTHNET ),
			'account-type-text'      => __( 'Account Type', PBCI_AUTHNET ),
			'business-checking-text' => __( 'Business Checking', PBCI_AUTHNET ),
			'checking-text'          => __( 'Checking', PBCI_AUTHNET ),
			'savings-text'           => __( 'Savings', PBCI_AUTHNET ),
			'name-on-account-text'   => __( 'Name on Account', PBCI_AUTHNET ),
			'routing-number-text'    => __( 'Routing Number', PBCI_AUTHNET ),
			'account-number-text'    => __( 'Account Number', PBCI_AUTHNET )
		);
...
EOF;
	...
	function setBillingAddress() {
		if ( isGood( $this->cart_data['billing_address']['address'] ) ) {
			$this->auth->address = $this->cart_data['billing_address']['address'];
		}
		if ( isGood( $this->cart_data['billing_address']['city'] ) ) {
			$this->auth->city = $this->cart_data['billing_address']['city'];
		}
		if ( isGood( $this->cart_data['billing_address']['state'] ) ) {
			$this->auth->state = $this->cart_data['billing_address']['state'];
		}
		if ( isGood( $this->cart_data['billing_address']['post_code'] ) ) {
			$this->auth->zip = $this->cart_data['billing_address']['post_code'];
		}
		if ( isGood( $this->cart_data['billing_address']['first_name'] ) ) {
			$this->auth->first_name = $this->cart_data['billing_address']['first_name'];
		}
		if ( isGood( $this->cart_data['billing_address']['last_name'] ) ) {
			$this->auth->last_name = $this->cart_data['billing_address']['last_name'];
		}
		if ( isGood( $this->cart_data['billing_address']['country'] ) ) {
			$this->auth->country = $this->cart_data['billing_address']['country'];
		}
		if ( isGood( $this->cart_data['email_address'] ) ) {
			$this->auth->email = $this->cart_data['email_address'];
		}
	}

	function setShippingAddress() {
		if ( isGood( $this->cart_data['shipping_address']['address'] ) ) {
			$this->auth->ship_to_address = $this->cart_data['shipping_address']['address'];
		}
		if ( isGood( $this->cart_data['shipping_address']['city'] ) ) {
			$this->auth->ship_to_city = $this->cart_data['shipping_address']['city'];
		}
		if ( isGood( $this->cart_data['shipping_address']['state'] ) ) {
			$this->auth->ship_to_state = $this->cart_data['shipping_address']['state'];
		}
		if ( isGood( $this->cart_data['shipping_address']['post_code'] ) ) {
			$this->auth->ship_to_zip = $this->cart_data['shipping_address']['post_code'];
		}
		if ( isGood( $this->cart_data['shipping_address']['first_name'] ) ) {
			$this->auth->ship_to_first_name = $this->cart_data['shipping_address']['first_name'];
		}
		if ( isGood( $this->cart_data['shipping_address']['last_name'] ) ) {
			$this->auth->ship_to_last_name = $this->cart_data['shipping_address']['last_name'];
		}
		if ( isGood( $this->cart_data['shipping_address']['country'] ) ) {
			$this->auth->ship_to_country = $this->cart_data['shipping_address']['country'];
		}
	}
	...
	function setSubscription() {
		...
		foreach ( $this->cart_items as $itemIndex => $item ) {
			if ( isset( $item['is_recurring'] ) && (int) $item['is_recurring'] > 0 ) {
				...
				if ( isGood( $_REQUEST['payType'] ) && $_REQUEST['payType'] = 'checkForms' ) {
				...
				if ( $this->response->isOk() ) {
					...
					$this->set_error_message( __( 'Failed to Authorize the Subscription', PBCI_AUTHNET ) );
					$this->return_to_checkout();
...
function form_auth_net() {
	...
	$authFormText = array(
		'api-login-id-text'               => __( 'API Login ID', PBCI_AUTHNET ),
		'transaction-key-text'            => __( 'Transaction Key', PBCI_AUTHNET ),
		'test-mode-text'                  => __( 'Test Mode', PBCI_AUTHNET ),
		'enable-cim-text'                 => sprintf( __( 'Enable %1$sCIM%2$s', PBCI_AUTHNET ), '<a href="http://www.authorize.net/solutions/merchantsolutions/merchantservices/cim/">', '</a>' ),
		'verify-first-capture-later-text' => __( 'Verify First, Capture Later', PBCI_AUTHNET )
	);
	$output       = <<<EOF
	<tr>
	  <td>
