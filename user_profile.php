...
if ( is_user_logged_in() ) {
	...
	function wpec_auth_net_user_profile_display_links($div){
		global $separator, $wpec_auth_net_user_profile_url;
		echo "{$div} <a href='{$wpec_auth_net_user_profile_url}'>".__( 'Saved Credit Card, Bank or Shipping Information', PBCI_AUTHNET )."</a> ";
	}
	...
	function wpec_auth_delete($type,$id){
		...
		if($type!='shippingadress'){
			...
			if($result === false){
				...
				wpsc_update_customer_meta( 'auth_net_message', __( 'Saved Payment Details Have Been Deleted.', PBCI_AUTHNET ) );
			}
		}else{
			...
			if($result === false){
                ...
                wpsc_update_customer_meta( 'auth_net_message', __( 'Saved Shipping Details Have Been Deleted.', PBCI_AUTHNET ) );
            }
		}
	}
	...
	function wpec_auth_net_user_profile_display(){
		...
		<h2><?php _e( 'Saved Credit Card, Bank and Shipping Information', PBCI_AUTHNET );?></h2>
		...
		<form action="<?php echo $wpec_auth_net_user_profile_url;?>" method="post">
			...
			<span class="sectionHeader"><?php _e( "Bank Accounts You've Saved For Easy Checkout.", PBCI_AUTHNET ); ?></span>
		...
		<form action="<?php echo $wpec_auth_net_user_profile_url;?>" method="post">
			...
			<span class="sectionHeader"><?php _e( "Credit Cards You've Saved For Easy Checkout.", PBCI_AUTHNET ); ?></span>
		...
		<form action="<?php echo $wpec_auth_net_user_profile_url;?>" method="post">
			...
			<span class="sectionHeader"><?php _e( "Shipping Addresses You've Saved For Easy Checkout.", PBCI_AUTHNET ); ?></span>
